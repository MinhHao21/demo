    ...
    AIZ.plugins = {
        ...
        slickCarousel: function () {
            $(".aiz-carousel").not(".slick-initialized").each(function () {
                ...
                var slidesToScroll = parseInt($this.attr("slide-to-scroll")) || 1;
                var autoplaySpeed = parseInt($this.attr("auto-play-speed")) || 2000;
                var disabledScreenWidth = parseInt($this.attr("disabled-screen")) || 0;

                const screenWidth = document.body.offsetWidth;
                if (screenWidth <= disabledScreenWidth) {
                    $this.removeClass(['aiz-carousel', 'mobile-img-slick-height']);
                    let children = $this.children();
                    const childrenClassWhenDisabled = $this.attr("children-class-when-disabled") || "";
                    for (let i = 0; i < children.length; ++i) {
                        $(children[i]).addClass(childrenClassWhenDisabled);
                    }
                    return;
                }
                ...
                if (!isNaN(currentVal)) {
                    if (type == "minus") {
                        if (currentVal > input.attr("min")) {
                            let newVal = Math.round((currentVal - min) * 10) / 10
