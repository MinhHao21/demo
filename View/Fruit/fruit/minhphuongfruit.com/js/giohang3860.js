...
function doAddCart($name,$id,$qty,$color,$size){
	...
	initAjax({
		...
		success:function(data){
			...
			showMsg("error","Thêm sản phẩm "+$name+" vào giỏ không thành công");
		}
			NProgress.done();
			//showCartMini();
		}
...
function doAddCart1($id){
	NProgress.start();
	$qty=$('#qty').val();
	initAjax({
		url:"ajax/add-cart.html",
		async:false,
		data:{id:$id,qty:$qty,color:'',size:''},
		success:function(data){
			$(".num-cart").html(data);
			setTimeout(function(){
...
$().ready(function(){
	...
	$("body").on('click', '.control_qty_cart button', function() {
		let act = $(this).attr('data-act');
		let ele_qty = $(this).parent().find('input');
		let qty = parseInt(ele_qty.val());
		if(act=='add'){
			qty = parseInt(qty) + 1;
		}else{
